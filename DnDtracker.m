...
inputFolder = uigetdir('./', 'Select the data folder to track larvae.');
...
filesArray = dir(strcat(inputFolder,'image*.jpg'));

% sorting the indics of file names
[~, reindex] = sort( str2double( regexp( {filesArray.name}, '\d+', 'match', 'once' )));
filesArray = filesArray(reindex) ;

%% setting parameters

fps = 2;  % frames per second
tAnal = 4;   % total time for tracking larave in min
nLarvae = 6; % number of larvae to be tracked
larvalMinSize = 0.5; % threshold value for detecting minimum size larva in mm
larvalMaxSize = 8;  % threshold value for detecting maximum size larva in mm
maxFile = 60*fps*tAnal;  % total frames to be analyzed
thLevel = 0.1; % threshold for binarizing the images
...
for i = 1:nLarvae
title(sprintf('Mark the level of the agarose #%i by clicking.',i))
set(gca,'xtick',[],'ytick',[])
[x,y] = ginput(1);
agarLevel(i) = y;
...
% get centroids
for i = 1:nLarvae
COM = tracking(inputFolder,filesArray,maxFile,larvalMinSize,larvalMaxSize,scale,thLevel,i,cham(i,:),ibg);
